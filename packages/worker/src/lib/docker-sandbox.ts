...
async function copyFilesToContainer(
  ...
  // Extract parent dir and leaf dir from destDir (e.g. "/sandbox/input" → "/sandbox", "input")
  // Docker archive API requires the parent to exist; tar entries are extracted relative to `path`.
  // We upload to "/" with full paths in the tar to avoid "directory not found" errors.
