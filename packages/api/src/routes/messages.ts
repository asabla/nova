...
messagesRouter.post("/:conversationId/messages/stream", zValidator("json", streamSchema), async (c) => {
  ...
  const formattingInstruction = {
    ...
    content: [
      ...
      '- Color Palette (display colors): ```widget\n{"type":"colorpalette","title":"Brand Colors","params":{"colors":"#6366f1,#ec4899,#14b8a6","labels":"Primary,Accent,Teal"}}\n```',
      '- Checklist (interactive todo list): ```widget\n{"type":"checklist","title":"Tasks","params":{"items":"Design,Implement,Test,Deploy","checked":"0,1"}}\n```',
      '- Timeline (events in order): ```widget\n{"type":"timeline","title":"Milestones","params":{"events":"[{\\"date\\":\\"2025-01-15\\",\\"title\\":\\"Kickoff\\",\\"description\\":\\"Project started\\"},{\\"date\\":\\"2025-06-01\\",\\"title\\":\\"Launch\\"}]"}}\n```',
      '- Dice (random roller): ```widget\n{"type":"dice","title":"Dice Roller","params":{"type":"d6","count":"1"}}\n```',
      '- Unit Converter: ```widget\n{"type":"unitconverter","title":"Length","params":{"category":"length","from":"km","to":"mi","value":"10"}}\n```',
      '- Calendar (month view with highlights): ```widget\n{"type":"calendar","title":"March 2026","params":{"month":"2026-03","highlights":"2026-03-15,2026-03-21","labels":"{\\"2026-03-15\\":\\"Demo Day\\",\\"2026-03-21\\":\\"Retro\\"}"}}\n```',
      '- QR Code: ```widget\n{"type":"qrcode","title":"QR Code","params":{"data":"https://example.com","size":"200"}}\n```',
      '- Diff (code comparison): ```widget\n{"type":"diff","title":"Changes","params":{"original":"old code here","modified":"new code here","language":"typescript","mode":"unified"}}\n```',
      '- JSON Explorer (interactive tree): ```widget\n{"type":"jsonexplorer","title":"API Response","params":{"data":"{\\"key\\":\\"value\\",\\"nested\\":{\\"a\\":1}}"}}\n```',
      '- Code Display (show executed code + output): ```widget\n{"type":"codedisplay","title":"Result","params":{"code":"print(42)","language":"python","output":"42","status":"success"}}\n```',
      '- Currency (live exchange rates): ```widget\n{"type":"currency","title":"USD to EUR","params":{"from":"USD","to":"EUR","amount":"100"}}\n```',
      '- Stock (ticker display): ```widget\n{"type":"stock","title":"AAPL","params":{"symbol":"AAPL","price":"178.72","change":"+2.35","changePercent":"+1.33","range":"1m","sparkline":"165,168,170,172,175,178"}}\n```',
      '- YouTube (video embed): ```widget\n{"type":"youtube","title":"Video","params":{"videoId":"dQw4w9WgXcQ"}}\n```',
      '- Kanban (project board): ```widget\n{"type":"kanban","title":"Sprint Board","params":{"columns":"[{\\"id\\":\\"todo\\",\\"title\\":\\"To Do\\"},{\\"id\\":\\"done\\",\\"title\\":\\"Done\\"}]","cards":"[{\\"id\\":\\"1\\",\\"title\\":\\"Task 1\\",\\"columnId\\":\\"todo\\"}]"}}\n```',
      '- Quiz (interactive quiz with scoring): ```widget\n{"type":"quiz","title":"Quiz","params":{"title":"Trivia","questions":"[{\\"question\\":\\"Capital of France?\\",\\"options\\":[\\"London\\",\\"Paris\\",\\"Berlin\\"],\\"correctIndex\\":1}]"}}\n```',
      "",
      ...
      "- COLORPALETTE: Use when discussing colors, design systems, or brand guidelines. colors and labels are comma-separated.",
      "- CHECKLIST: Use when listing tasks, steps, or action items. checked is comma-separated 0-based indices of pre-checked items.",
      "- TIMELINE: Use when showing chronological events, project history, or milestones. events is a JSON array of {date, title, description?}.",
      "- DICE: Use for random generation, games, or decision-making. type: 'd6' (default), 'd20', 'coin', or 'custom'. count is number of dice. Default to d6 unless user asks for d20.",
      "- UNITCONVERTER: Use when converting units. category: 'length', 'weight', 'temperature', 'volume', 'speed'.",
      "- CALENDAR: Use when discussing dates, scheduling, or deadlines. highlights is comma-sep dates, labels is JSON mapping dates to labels.",
      "- QRCODE: Use when the user wants a QR code for a URL, text, or contact info.",
      "- DIFF: Use when comparing code versions or showing changes. mode: 'unified' or 'split'. Great for code reviews.",
      "- JSONEXPLORER: Use when showing API responses, config files, or structured data the user needs to explore.",
      "- CODEDISPLAY: Use when showing code that was or could be executed, along with its output. status: 'success' or 'error'.",
      "- CURRENCY: Use when asked about exchange rates or currency conversion. Fetches live rates from the Frankfurter API (api.frankfurter.dev) automatically — you do NOT need to fetch data yourself.",
      "- STOCK: Use when discussing stocks. You must provide the data in params (price, change, sparkline). Use web_search or fetch_url to get current stock data, then pass it to the widget. The widget does NOT fetch data itself.",
      "- YOUTUBE: Use when the user shares or asks about a YouTube video. Accepts videoId or url param.",
      "- KANBAN: Use when helping with project planning, task organization, or sprint boards. columns and cards are JSON arrays.",
