...
workspaceRoutes.post("/", async (c) => {
  ...
  await auditService.writeAuditLog({ orgId, userId, action: "workspace.create", resourceType: "workspace", resourceId: workspace.id });
...
workspaceRoutes.delete("/:id", async (c) => {
  ...
  await auditService.writeAuditLog({ orgId, userId, action: "workspace.delete", resourceType: "workspace", resourceId: c.req.param("id") });
...
workspaceRoutes.post("/:id/members", async (c) => {
  ...
  const membership = await workspaceService.addMember(c.req.param("id"), body.userId, body.role);
