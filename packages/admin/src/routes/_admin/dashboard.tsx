...
import { Building2, Users, MessageSquare, Zap, TrendingUp, Activity, ArrowUpRight, Clock } from "lucide-react";
...
function MetricCard({ label, value, icon: Icon, accentColor, dimColor, subtitle }: {
  ...
  return (
    <div
      className="relative overflow-hidden rounded-xl border p-5 transition-all duration-200 hover:border-opacity-60 group"
      style={{ background: "var(--color-surface-raised)", borderColor: "var(--color-border-subtle)" }}
    >
      <div className="flex items-start justify-between">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-wider font-mono" style={{ color: "var(--color-text-muted)" }}>
            {label}
          </p>
...
function SkeletonCard() {
  return <div className="rounded-xl h-[120px] skeleton" />;
}
...
function DashboardPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <MetricCard
            ...
            dimColor="var(--color-accent-blue-dim)"
          />
          <MetricCard
            label="Users"
            ...
            dimColor="var(--color-accent-green-dim)"
            subtitle={stats?.active_users_7d ? `${stats.active_users_7d} active this week` : undefined}
          />
          <MetricCard
            label="Conversations"
            ...
            dimColor="var(--color-accent-purple-dim)"
          />
          <MetricCard
            label="Messages"
            ...
            dimColor="var(--color-accent-amber-dim)"
          />
        </div>
      )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="rounded-xl border p-5" style={{ background: "var(--color-surface-raised)", borderColor: "var(--color-border-subtle)" }}>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-semibold uppercase tracking-wider font-mono" style={{ color: "var(--color-text-muted)" }}>Uptime</span>
            </div>
            <p className="text-2xl font-bold" style={{ color: "var(--color-text-primary)" }}>99.9%</p>
            <p className="text-xs mt-1" style={{ color: "var(--color-text-secondary)" }}>Platform availability</p>
          </div>
        </div>
      )}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ borderBottom: "1px solid var(--color-border-subtle)" }}>
                <th className="text-left px-5 py-3 text-[10px] font-semibold uppercase tracking-wider font-mono" style={{ color: "var(--color-text-muted)" }}>Organisation</th>
                <th className="text-left px-5 py-3 text-[10px] font-semibold uppercase tracking-wider font-mono" style={{ color: "var(--color-text-muted)" }}>Plan</th>
                <th className="text-right px-5 py-3 text-[10px] font-semibold uppercase tracking-wider font-mono" style={{ color: "var(--color-text-muted)" }}>Members</th>
                <th className="text-right px-5 py-3 text-[10px] font-semibold uppercase tracking-wider font-mono" style={{ color: "var(--color-text-muted)" }}>Messages</th>
                <th className="text-right px-5 py-3 text-[10px] font-semibold uppercase tracking-wider font-mono" style={{ color: "var(--color-text-muted)" }}>Tokens</th>
              </tr>
            </thead>
            <tbody>
              {usage.data.slice(0, 10).map((org: any) => (
                <tr key={org.orgId} className="row-hover transition-colors" style={{ borderBottom: "1px solid var(--color-border-subtle)" }}>
                  <td className="px-5 py-3">
                    <span className="font-medium" style={{ color: "var(--color-text-primary)" }}>{org.orgName}</span>
                  </td>
                  <td className="px-5 py-3">
                    <span className="inline-flex px-2 py-0.5 rounded text-[10px] font-mono font-medium" style={{ background: "var(--color-surface-overlay)", color: "var(--color-text-secondary)" }}>
      ...
      {!isLoading && (!stats || (stats.org_count === 0 && stats.user_count === 0)) && (
        <div className="rounded-xl border p-12 text-center" style={{ background: "var(--color-surface-raised)", borderColor: "var(--color-border-subtle)" }}>
          ...
          <p className="text-sm max-w-md mx-auto mb-6" style={{ color: "var(--color-text-secondary)" }}>
            Your platform is ready. Create your first organisation to get started, or configure model providers to enable AI capabilities.
          </p>
          <div className="flex items-center justify-center gap-3">
            <a href="/organisations" className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium text-white transition-colors" style={{ background: "var(--color-accent-blue)" }}>
              Create Organisation <ArrowUpRight className="h-3.5 w-3.5" />
            </a>
            <a href="/providers" className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium border transition-colors" style={{ borderColor: "var(--color-border-default)", color: "var(--color-text-secondary)" }}>
              Configure Providers
            </a>
          </div>
        </div>
