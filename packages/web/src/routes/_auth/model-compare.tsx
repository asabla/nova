  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 px-6 py-4 border-b border-border">
        ...
        <h1 className="text-lg font-semibold text-text">{t("modelCompare.title", "Model Comparison")}</h1>
      </div>

      {/* Error loading models */}
      {isError && (
        <div className="flex items-center gap-2 px-6 py-3 bg-danger/5 border-b border-danger/20">
          <XCircle className="h-4 w-4 text-danger" aria-hidden="true" />
          <span className="text-sm text-danger">{t("modelCompare.loadError", "Failed to load models")}</span>
        </div>
      )}
      ...
      {showDiff && (
        <div className="border-t border-border px-6 py-4 max-h-64 overflow-y-auto bg-surface-secondary">
          <h3 className="text-sm font-medium text-text mb-2">{t("modelCompare.responseDiff", "Response Diff")}</h3>
          <div className="font-mono text-xs space-y-0.5">
            ...
            {diffLines.length === 0 && (
              <p className="text-text-tertiary">{t("modelCompare.identical", "Responses are identical.")}</p>
            )}
          </div>
        </div>
      )}
        ...
        <div className="flex gap-3 items-end">
          ...
          <div className="flex flex-col gap-2">
            ...
            >
              ...
              {t("modelCompare.compare", "Compare")}
            </Button>
            <Button
              variant="secondary"
              size="sm"
            ...
            >
              ...
              {showDiff ? t("modelCompare.hideDiff", "Hide Diff") : t("modelCompare.compareResults", "Compare Results")}
            </Button>
          </div>
        </div>
        {modelA && modelB && modelA === modelB && (
          <p className="text-xs text-warning mt-2">{t("modelCompare.sameModelWarning", "Select two different models to compare.")}</p>
        )}
      </div>
    </div>
  );
    ...
    <div className="flex-1 flex flex-col min-w-0">
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-surface-secondary border-b border-border">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-text truncate">{modelName || t("modelCompare.notSelected", "Not selected")}</span>
        </div>
        {result.streaming && (
          <span className="text-xs text-primary animate-pulse">{t("modelCompare.streaming", "Streaming...")}</span>
        )}
      </div>
        ...
        <div className="flex items-center gap-4 px-4 py-1.5 bg-surface-tertiary border-b border-border text-xs text-text-secondary">
          ...
          {result.totalTokens > 0 && (
            <span className="flex items-center gap-1">
              ...
              {result.totalTokens} {t("modelCompare.tokens", "tokens")} ({result.promptTokens}p / {result.completionTokens}c)
            </span>
          )}
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3">
        {result.error ? (
          ...
          <p className="text-sm text-text-tertiary text-center py-12">
            {t("modelCompare.emptyState", "Select a model and enter a prompt to begin.")}
          </p>
        ) : (
