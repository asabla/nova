...
import { Sparkles, Eye, EyeOff, Mail, ArrowRight, Loader2, X } from "lucide-react";
    ...
    try {
      if (mode === "magic-link") {
        ...
        if (resp.ok) {
          ...
          setError(err.message ?? t("auth.magicLinkFailed", "Failed to send magic link"));
        }
        return;
      }
      ...
      if (data) {
        ...
        try {
          ...
          setError(t("auth.orgInitFailed", "Signed in, but failed to initialize organization. Please try refreshing."));
          return;
        }
        navigate({ to: "/" });
      }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-surface px-4">
        <div className="w-full max-w-sm text-center">
          ...
          <h1 className="text-xl font-bold text-text">{t("auth.checkYourEmail", "Check your email")}</h1>
          <p className="text-sm text-text-secondary mt-2">
            {t("auth.magicLinkDescription", "We sent a magic link to {{email}}. Click the link in the email to sign in.", { email })}
          </p>
          <button
      ...
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Button type="submit" variant="primary" size="md" className="w-full" loading={loading}>
            {mode === "magic-link" ? (
              <><Mail className="h-4 w-4" /> {t("auth.sendMagicLink", "Send Magic Link")}</>
            ) : (
              t("auth.signIn")
            )}
          </Button>
